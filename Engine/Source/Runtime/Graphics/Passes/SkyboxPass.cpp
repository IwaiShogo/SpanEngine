...
namespace Span
{
	...
	void SkyboxPass::Render(Renderer* renderer, ID3D12GraphicsCommandList* cmd, const EnvironmentSettings& env, const Matrix4x4& view, const Matrix4x4& proj, const Vector3& camPos)
	{
		if (!m_pso || !env.UseProceduralSky) return;

		struct SkyboxCameraCB { Matrix4x4 InvView; Matrix4x4 InvProj; Vector3 CamPos; float padding; };
		struct SkyboxSettingsCB { Vector3 TopColor; float p1; Vector3 HorizonColor; float p2; Vector3 BottomColor; float p3; };
		...
		setData.TopColor = Vector3(env.SkyTopColor[0], env.SkyTopColor[1], env.SkyTopColor[2]);
		setData.HorizonColor = Vector3(env.SkyHorizonColor[0], env.SkyHorizonColor[1], env.SkyHorizonColor[2]);
		setData.BottomColor = Vector3(env.SkyBottomColor[0], env.SkyBottomColor[1], env.SkyBottomColor[2]);
		...
		if (cbCamAddress == 0 || cbSetAddress == 0) return; // バッファ上限
